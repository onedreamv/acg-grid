...
- 绝不裁剪、不拉伸图片：封面统一 `object-fit: contain`。卡片宽高比取封面原始比例，但极端窄图（aspect < `MIN_CARD_ASPECT`，见 `src/lib/metrics.ts`）在布局时垫宽至下限，图片居中、两侧留白（`LETTERBOX_BG`），保证元数据层可容纳态度 + 徽章。首次访问初始化 10 张占位卡（随机 2:3 / 3:4 / 4:3），其中有且仅有 1 张 16:9。
